...
export async function runAdvisorAgent(state: CareerAgentState): Promise<CareerAgentState> {
  ...
  let summary = state.assessment.resume_filename
    ? `Based on uploaded resume (${state.assessment.resume_filename}), we evaluated your technical background and recommend building your skills towards becoming a ${research.role}.`
    : `Based on your profile, we recommend building your skills towards becoming a ${research.role}. This role matches your interests and builds on your existing knowledge.`;

  let reasoning = [
    state.assessment.resume_filename 
      ? `Uploaded resume (${state.assessment.resume_filename}) verified candidate's technical background and stack.`
  ...
  if (process.env.AI_API_KEY) {
    try {
      ...
      const prompt = `
        ...
        Resume uploaded: ${state.assessment.resume_filename || 'None'}
        Resume Context: ${state.assessment.resume_text ? state.assessment.resume_text.substring(0, 1000) : 'None'}
